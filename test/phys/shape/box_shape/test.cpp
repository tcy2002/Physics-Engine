...
void testConstruct() {
    ...
    ASSERT_EQUAL_INT(box.getType(), ShapeType::ST_Box)
...
void testAABB() {
    ...
    transform.setRotation(pe::Vector3::UnitY(), PE_PI / 4);
...
void testProject() {
    ...
    box.project(pe::Transform::Identity(), pe::Vector3::UnitY(), min, max, minPoint, maxPoint);
    ASSERT_EQUAL(min, -1.)
    ASSERT_EQUAL(max, 1.)
    ASSERT_EQUAL(minPoint.y(), -1.)
    ASSERT_EQUAL(maxPoint.y(), 1.)

    box.project(pe::Transform::Identity(), pe::Vector3::UnitX(), min, max, minPoint, maxPoint);
    ASSERT_EQUAL(min, -0.5)
    ASSERT_EQUAL(max, 0.5)
    ASSERT_EQUAL(minPoint.x(), -0.5)
    ASSERT_EQUAL(maxPoint.x(), 0.5)

    box.project(pe::Transform::Identity(), pe::Vector3::UnitZ(), min, max, minPoint, maxPoint);
    ...
    box.project(transform, pe::Vector3::UnitY(), min, max, minPoint, maxPoint);
    ASSERT_EQUAL(min, 1.)
    ASSERT_EQUAL(max, 3.)
    ASSERT_EQUAL(minPoint.y(), 1)
    ASSERT_EQUAL(maxPoint.y(), 3)

    box.project(transform, pe::Vector3::UnitX(), min, max, minPoint, maxPoint);
    ASSERT_EQUAL(min, 2. - sqrt(2.))
    ASSERT_EQUAL(max, 2. + sqrt(2.))
    ASSERT_EQUAL(minPoint.x(), 2. - sqrt(2.))
    ASSERT_EQUAL(maxPoint.x(), 2. + sqrt(2.))

    box.project(transform, pe::Vector3::UnitZ(), min, max, minPoint, maxPoint);
    ASSERT_EQUAL(min, 2. - sqrt(2.))
    ASSERT_EQUAL(max, 2. + sqrt(2.))
