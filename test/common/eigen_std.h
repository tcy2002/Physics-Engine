...
ASSERT_VECTOR3_EQUAL(to.getOrigin(), origin); } while (0)
...
ASSERT(EQUAL(ao.x, bo.x()) && EQUAL(ao.y, bo.y()) && EQUAL(ao.z, bo.z()) && \
       ...
       "vector3 not equal to eigen"); } while (0)
...
ASSERT(EQUAL(ao[0][0], bo(0, 0)) && EQUAL(ao[0][1], bo(0, 1)) && EQUAL(ao[0][2], bo(0, 2)) && \
       ...
       "matrix3 not equal to eigen"); } while (0)

#define ASSERT_TRANSFORM_EIGEN_EQUAL(t, b, o) \
do { auto&& to = t; auto&& basis = b; auto&& origin = o; \
ASSERT_MATRIX3_EIGEN_EQUAL(to.getBasis(), basis); \
ASSERT_VECTOR3_EIGEN_EQUAL(to.getOrigin(), origin); } while (0)
