PEReal& Vector3::operator[](int i) {
    return (&x)[i];
}

PEReal Vector3::operator[](int i) const {
    return (&x)[i];
}
...
Vector3 Vector3::operator*(PEReal r) const {
    return {x * r, y * r, z * r};
}
...
Vector3& Vector3::operator*=(PEReal r) {
    x *= r;
    y *= r;
    z *= r;
    return *this;
...
PEReal Vector3::dot(const Vector3& v) const {
    return x * v.x + y * v.y + z * v.z;
}
...
Vector3 Vector3::rotate(const Vector3& axis, PEReal theta) const {
    auto n = axis.normalized();
    auto s = sin(theta), c = cos(theta);
    return *this * c + n.cross(*this) * s + n * dot(n) * (1 - c);
}

PEReal Vector3::angle(const Vector3& other) const {
