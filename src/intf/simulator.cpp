...
namespace pe_intf {
    ...
    public:
        ...
        void download(const std::string& path) {
            ...
            /*for (int i = 0; i < _mesh_counter; i++) {
                ...
                _model.nodes[i].translation.push_back(PE_F(_animation_translation[i][0][0]));
                _model.nodes[i].translation.push_back(PE_F(_animation_translation[i][0][1]));
                _model.nodes[i].translation.push_back(PE_F(_animation_translation[i][0][2]));
                _model.nodes[i].rotation.push_back(PE_F(_animation_rotation[i][0].x()));
                _model.nodes[i].rotation.push_back(PE_F(_animation_rotation[i][0].y()));
                _model.nodes[i].rotation.push_back(PE_F(_animation_rotation[i][0].z()));
                _model.nodes[i].rotation.push_back(PE_F(_animation_rotation[i][0].w()));
            }*/
            ...
            gltf.WriteGltfSceneToFile(&_model, path, true, true, true, true);
        }
    ...
    void Simulator::start() {
        ...
        auto dir = save_path + "/program-" + std::to_string(PE_I(COMMON_GetTickCount())) + "/";
...
#       endif
        ...
        while (true) {
            ...
            if ((use_gui && Viewer::getKeyState('n') == 0) || saving) {
            ...
            auto blocking_time = COMMON_GetMicroTickCount() - block_start;

            ++frame;
            auto actual_tick = COMMON_GetMicroTickCount() - start - blocking_time;
            if (frame >= max_frame) {
                break;
            }
            if (actual_tick < target_tick) {
            ...
            total_tick += COMMON_GetMicroTickCount() - start - blocking_time;
        }

        running = false;
        if (should_save) {
            writer.download(dir + std::to_string(COMMON_GetTickCount()) + ".glb");
        }
