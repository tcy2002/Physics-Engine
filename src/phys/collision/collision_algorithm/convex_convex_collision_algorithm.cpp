...
#   define PE_CONVEX_CONVEX_USE_METHOD1 false
    ...
    static pe::Real testSingleFaceDirectionPolyhedronVsPolyhedron(const pe_phys_shape::Shape* shape_a, const pe_phys_shape::Shape* shape_b,
                                                           const pe::Mesh& mesh_a, const pe::Mesh& mesh_b,
    ...
    static pe::Real testFacesDirectionPolyhedronVsPolyhedron(const pe_phys_shape::Shape* shape_a, const pe_phys_shape::Shape* shape_b,
                                                      const pe::Mesh& mesh_a, const pe::Mesh& mesh_b,
                                                      const pe::Transform &trans_a2b,
        ...
        for (uint32_t f = 0; f < mesh_a.faces.size(); f++) {
            ...
            if (penetrationDepth <= pe::Real(0.0)) {
                min_face_index = f;
                return penetrationDepth;
            }
    ...
    static COMMON_FORCE_INLINE bool testGaussMapArcsIntersect(const pe::Vector3& a, const pe::Vector3& b, const pe::Vector3& c, const pe::Vector3& d,
        ...
        return cba * dba < pe::Real(0.0) && adc * bdc < pe::Real(0.0) && cba * bdc > pe::Real(0.0);
    }

    static COMMON_FORCE_INLINE bool testEdgesBuildMinkowskiFace(const pe::Mesh& mesh_a, const pe::Mesh& mesh_b,
                                            const pe_phys_shape::UniqueEdge& edge_a, const pe_phys_shape::UniqueEdge& edge_b,
                                            const pe::Transform& trans_a2b) {
    ...
    static COMMON_FORCE_INLINE pe::Real computePlaneSegmentIntersection(const pe::Vector3& seg_a, const pe::Vector3& seg_b, const pe::Real plane_d, const pe::Vector3& plane_normal) {
        const pe::Real parallel_epsilon = pe::Real(0.0001);
        pe::Real t = pe::Real(-1);
    ...
    static COMMON_FORCE_INLINE void clipPolygonWithPlane(pe::Array<pe::Vector3>& polygon_vertices, const pe::Vector3& plane_point,
        ...
        for (int v_end_index = 0; v_end_index < nb_input_vertices; v_end_index++) {
    ...
    static bool computePolyhedronVsPolyhedronFaceContactPoints(bool is_min_penetration_face_normal_polyhedron1,
                                                        const pe::Mesh& mesh_a, const pe::Mesh& mesh_b,
                                                        const pe::Transform& trans_a, const pe::Transform& trans_b,
        ...
        int incident_face_index = -1;
        {
            ...
            for (int i = 0; i < nb_faces; i++) {
                ...
                if (support_value > max_support_value) {
                    ...
                    min_dot_product = PE_REAL_MAX;
                }
                const pe::Real dot_product = incident_polyhedron->faces[i].normal.dot(axis_incident_space);
                if (PE_APPROX_EQUAL(support_value, max_support_value) && dot_product < min_dot_product) {
        ...
        for (uint32_t i = 0; i < nb_clip_polygon_vertices; i++) {
            ...
            if (penetration_depth > pe::Real(0.0)) {
    ...
    static COMMON_FORCE_INLINE void computeClosestPointBetweenTwoSegments(const pe::Vector3& seg1_point_a, const pe::Vector3& seg1_point_b,
        ...
        if (a <= PE_EPS) {
            ...
            t = PE_CLAMP(f / e, pe::Real(0.0), pe::Real(1.0));
        } else {
            ...
            if (e <= PE_EPS) {
                ...
                if (t < pe::Real(0.0)) {
                    t = pe::Real(0.0);
                    s = PE_CLAMP(-c / a, pe::Real(0.0), pe::Real(1.0));
                } else if (t > pe::Real(1.0)) {
                    t = pe::Real(1.0);
                    s = PE_CLAMP((b - c) / a, pe::Real(0.0), pe::Real(1.0));
                }
            }
        }
    ...
    const pe::Real SEPARATING_AXIS_RELATIVE_TOLERANCE = pe::Real(1.002);
    const pe::Real SEPARATING_AXIS_ABSOLUTE_TOLERANCE = pe::Real(0.0005);

    static bool findSeparatingAxis2(const pe_phys_shape::Shape* shape_a, const pe_phys_shape::Shape* shape_b,
                                    const pe::Mesh& mesh_a, const pe::Mesh& mesh_b,
                                    const pe::Array<pe_phys_shape::UniqueEdge>& unique_edges_a,
                                    const pe::Array<pe_phys_shape::UniqueEdge>& unique_edges_b,
                                    const pe::Transform &trans_a, const pe::Transform &trans_b,
        ...
        if (penetration_depth1 <= pe::Real(0.0)) {
        ...
        if (penetration_depth2 <= pe::Real(0.0)) {
            // We have found a separating axis
            return true;
        }
