    ...
    public:
            ...
            for (auto& contact : contacts) {
                ...
                for (int pi = 0; pi < contact->getPointSize(); pi++) {
                    ...
                    /*const pe::VectorX grad = use_stored_constraints ?
                        _gradients.segment<2>(contact_i, 0) :
                        _fc->gradient(f);
                ...
                if (use_stored_constraints) {
                    rl = lambda.cwiseProduct(_constraint_val) + pe::VectorX::Ones(contact_size) * mu;
                } else {
                    ...
                    rl = lambda.cwiseProduct(c) + pe::VectorX::Ones(contact_size) * mu;
                }
            }
        }
        ...
        void calcConstraints(const pe::VectorX& vel, const pe::VectorX& forces, const pe::VectorX& lambda) override {
            for (size_t i = 0; i < forces.size() / 3; i++) {
                ...
                _gradients.block<2, 1>(i, 0) = _fc->gradient(f);
        ...
        void linearSystemAddition(const pe::Array<pe_phys_collision::ContactResult*>& contacts,
            ...
            _inv_constraint_val = R(1) / (_constraint_val.array() - eps * lambda.array());
            ...
            for (size_t i = 0; i < contact_size; i++) {
                ...
                auto reg = pe::Matrix2::Identity() * eps;
                //reg += l_mats[i];
                //reg += lambda[2 * i + 1] * _hessians.block<2, 2>(i * 2, 0);
                // TODO: solve svd
            }
        }
