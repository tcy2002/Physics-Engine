...
common::Matrix3x3<float> Viewer::convertMatrix3(pe::Matrix3 matrix) {
    return {
        PE_F(matrix[0][0]), PE_F(matrix[0][1]), PE_F(matrix[0][2]),
...
static simple_viewer::ObjType mapShapeType(pe_physics_shape::ShapeType type) {
    switch (type) {
        case pe_physics_shape::ShapeType::ST_Box:
            return simple_viewer::ObjType::OBJ_CUBE;
        case pe_physics_shape::ShapeType::ST_Sphere:
            return simple_viewer::ObjType::OBJ_SPHERE;
        ...
        default:
            return simple_viewer::ObjType::OBJ_NONE;
    }
}
...
void Viewer::updateTransform(int id, pe_physics_shape::ShapeType type, const pe::Transform &transform) {
...
void Viewer::updateColor(int id, pe_physics_shape::ShapeType type, const pe::Vector3 &color) {
    if (_obj_map[id] == mapShapeType(type)) {
        updateObj(simple_viewer::ObjUpdateParam(
