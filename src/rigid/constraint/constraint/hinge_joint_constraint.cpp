...
namespace pe_phys_constraint {
    ...
    void HingeJointConstraint::initSequentialImpulse(const ConstraintParam &param) {
        ...
        pe_phys_collision::ContactPoint::getOrthoUnits(_w_axis_a, _w_t_a[0], _w_t_a[1]);
        ...
        const pe::Matrix3 inertia_sum = _object_a->getWorldInvInertia() + _object_b->getWorldInvInertia();
        _jmj_inv_hinge[0] = pe::Real(1.0) / _w_t_a[0].dot(inertia_sum * _w_t_a[0]);
        _jmj_inv_hinge[1] = pe::Real(1.0) / _w_t_a[1].dot(inertia_sum * _w_t_a[1]);

        pe::Vector3 u = _w_axis_a.cross(_w_axis_b);
    ...
    void HingeJointConstraint::iterateSequentialImpulse(int iter) {
        ...
        const pe::Vector3 tmp_ball_impulse = _rhs_ball - _jmj_inv_ball * (vel_a - vel_b);
        _object_a->applyTempImpulse(_r_a, tmp_ball_impulse);
        _object_b->applyTempImpulse(_r_b, -tmp_ball_impulse);

        const pe::Vector3 w_a = _object_a->getTempAngularVelocity();
        const pe::Vector3 w_b = _object_b->getTempAngularVelocity();
        pe::Real hinge_impulse0 = _rhs_hinge[0] - _jmj_inv_hinge[0] * _w_t_a[0].dot(w_a - w_b);
        pe::Real hinge_impulse1 = _rhs_hinge[1] - _jmj_inv_hinge[1] * _w_t_a[1].dot(w_a - w_b);
